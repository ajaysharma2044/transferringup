...
Deno.serve(async (req: Request) => {
  ...
  try {
    const lead: ConsultationLead = await req.json();
...
High School GPA: ${lead.highSchoolGPA}
College GPA: ${lead.collegeGPA || 'N/A'}
Target Schools: ${lead.targetSchools}
...
${lead.biggestChallenge}
    ...
    const result = await sendGmailEmail(
      accessToken,
      NOTIFICATION_EMAIL,
      `New Consultation Lead: ${lead.firstName} ${lead.lastName}`,
      emailBody
    );
